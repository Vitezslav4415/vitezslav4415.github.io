...
var $board = $('.board'),
    ...
    setBoard = function (conf) {
        ...
        // Loyalty tokens
        value = conf.loyalty.split('\n');
        for(var i = 0; i < value.length; i += 1) {
            htmlString += '<div class="loyalty pos-' + value[i].toLowerCase().toLowerCase().replace(/ - port$/, '-harbor').replace(/([' ]|^the )/g, '') + '"></div>';
        }
        // VSB and Raven token
